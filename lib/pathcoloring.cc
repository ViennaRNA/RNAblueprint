...
namespace design {
    namespace detail {
        ...
        template <typename RG>
        unsigned long long generate_seq_parts(Sequence& sequence, Parts parts, int length, RG* rand_ptr) {
            unsigned long long max_number_of_sequences = 0;
            
            Pairing pairing(length + 1);
            
            // remember which end we picked from the previous part.
            int new_start = -1;
            foreach(auto p : parts) {
                max_number_of_sequences += pairing.get(p.length, p.start, p.end);
                if (new_start == -1) {
                    new_start = p.start;
                }
                
                sequence.pop_back();
                unsigned long long real_nos = generate_path_seq(sequence, new_start, p.end, p.length, rand_ptr);
                if (real_nos == 0) {
                    std::cerr << "It is not possible to color with these sequence constraints!" << std::endl;
                    exit(1);
                }
                new_start = sequence.back();
        ...
        unsigned long long generate_cycle_seq(Sequence& sequence, int first, int length, RG* rand_ptr) {
            ...
            if (first < A_Size) {
                ...
                generate_seq_parts(Sequence& sequence, Parts parts, int length, RG* rand_ptr)
            }
        ...
        unsigned long long color_path_cycle_graph(Graph& g, RG* rand_ptr) {
            ...
            BGL_FORALL_VERTICES_T(v, g, Graph) {
                ...
                std::cerr << "(" << boost::get(boost::vertex_color_t(), g, v) << ")" << std::endl;
            ...
            if (ends.size() == 2) {
                // it is a path!
                // check if any of the non-end vertices have assigned colors
                if (colored_bases.size() > 0) {
                    // split path at sequence constraints and color in between, then glue together again.
            ...
            BGL_FORALL_VERTICES_T(v, g, Graph) {
                if (g[v].base < A_Size && g[v].base != sequence[boost::get(boost::vertex_color_t(), g, v)]) {
                    std::cerr << "Tried to color following vertex with a base, but it is already colored with another one! "
                            << boost::get(boost::vertex_color_t(), g, v) << "/" << enum_to_char(g[v].base) << ", new color: " << enum_to_char(sequence[boost::get(boost::vertex_color_t(), g, v)]) << std::endl;
                    exit(1);
                }
                g[v].base = sequence[boost::get(boost::vertex_color_t(), g, v)];
            }
