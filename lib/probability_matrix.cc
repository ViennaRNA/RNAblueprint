...
namespace design {
    ...
    /*    
    ProbabilityMatrix::fill_for_block() {
            ...
            for (boost::tie(ear, ear_end) = (g).children(); ear != ear_end; ++ear) {
                ...
                std::vector<ProbabilityKey> key_combinations; // this is what we want to fill in the recursion
                ProbabilityKey mykey; // helper to recursively build the posibilities
        ...
        unsigned long long ProbabilityMatrix::get_probability(ProbabilityKey mykey, Graph& g, std::set<Vertex>& Ak, std::set<Vertex>& Ai, unsigned int k) {
        ...
        void ProbabilityMatrix::make_sum_of_sum(Graph& g,
                std::set<Vertex>& Ai,
                ProbabilityKey& mykey, ProbabilityKey& lastkey,
                std::vector<SubProbability>& sub_probabilities,
                unsigned int k,
        ...
        int ProbabilityMatrix::get_color_from_key(ProbabilityKey& mykey, ProbabilityKey& lastkey, int vertex) {
        ...
        unsigned long long ProbabilityMatrix::calculate_probability(ProbabilityKey& mykey, MyKey& lastkey,
                std::vector<SubProbability>& sub_probabilities) {
        ...
        void ProbabilityMatrix::calculate_combinations(std::set<int>& Ak, MyKey& mykey, std::vector<ProbabilityKey>& key_combinations) {
        ...
        unsigned long long ProbabilityMatrix::get_sum(int k, ProbabilityKey mykey, ProbabilityKey lastkey, std::vector<ProbabilityKey>& key_combinations, ProbabilityMap& probabilities) {
            ProbabilityKey tempkey; // helper to build all combinations
            ...
            if (cAk.size() != 0) {
                ...
                key_combinations.push_back(ProbabilityKey());
            }

            // need to add the static key-elements in mykey to our keys in the key_combinations
            for (std::vector<ProbabilityKey>::iterator it = key_combinations.begin(); it != key_combinations.end(); ++it) {
