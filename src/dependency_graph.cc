...
DependencyGraph::DependencyGraph (std::vector<std::string> structures) {
	...
	if (debug) { std::cerr << "Initializing DependencyGraph..." << std::endl; }
	...
	if (debug) { std::cerr << "Calculate number of sequences for dependency graph..." << std::endl; }
...
: subgraph(g), type(t) {
	...
	if (debug) { std::cerr << "Initializing GraphComponent..." << std::endl; }
	...
	if (debug) { print_all_vertex_names(subgraph, "Initialized a GraphComponent"); }
